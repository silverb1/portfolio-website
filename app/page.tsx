...
export default function Portfolio() {
  ...
  return (
    <div className="relative w-full min-h-screen overflow-hidden bg-background">
      ...
      {/* Content */}
      <div className="relative z-10 min-h-screen flex items-center justify-center p-6 md:p-8 lg:p-10">
        <div className="w-full lg:w-3/4 flex flex-col lg:flex-row gap-8 lg:gap-30">
          {/* Left column */}
          <div className="w-full lg:w-2/3 flex flex-col justify-center lg:pr-8">
            {/* Name */}
            <h1
              className={`text-4xl md:text-5xl lg:text-6xl font-bold text-primary mb-2 transition-all duration-1000 transform ${
            ...
            {/* Description */}
            <p
              className={`text-sm md:text-base lg:text-lg text-secondary mb-8 leading-relaxed max-w-md transition-all duration-1000 transform delay-300 ${
                isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              }`}
            >
            ...
            {/* Skills */}
            <div
              className={`flex flex-wrap gap-3 mb-8 lg:mb-0 transition-all duration-1000 transform delay-500 ${
                isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              }`}
            >
              {["Python", "Java", "C++", "C#", "React", "TypeScript", "Node.js"].map((skill) => (
                <div
                  key={skill}
                  className="px-4 py-3 rounded-lg border border-accent/30 text-secondary hover:border-accent/60 transition-colors duration-300 text-sm md:text-base"
                >
                  {skill}
                </div>
              ))}
          ...
          <div className="w-full lg:w-1/2 flex flex-col gap-4 lg:pl-8">
            <h2
              className={`text-2xl md:text-3xl font-bold text-primary mb-2 transition-all duration-1000 transform ${
                isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              }`}
            >
              Projects
            ...
            {projects.map((project, index) => (
              ...
              >
                <h3 className="text-lg md:text-xl font-semibold text-primary mb-2 group-hover:text-accent transition-colors">
                  {project.name}
                </h3>
                <p className="text-xs md:text-sm text-accent mb-3">{project.languages}</p>
